import type { ChatMessage, ChatMessageRole, RunCardMeta, RunStepMeta } from './types';
...
function parseRunMetaFromDataJson(dataJson: string, fallbackHeader: string): RunCardMeta {
  ...
  try {
    ...
    return {
      ...
      stepStatus: (rp.stepStatus as RunStepMeta['status'] | undefined) ?? null,
  ...
  try {
    ...
    return parsed.runProgressHistory
      ...
      .map((entry) => {
        ...
        const status: RunStepMeta['status'] =
          normalized === 'failed'
            ? 'failed'
